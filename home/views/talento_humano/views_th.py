...
@login_required
@user_passes_test(lambda u: u.groups.filter(name_in=['Director Talento Humano', 'Secretaria Talento Humano']).exists())
def director_talento_humano(request):
    return render(request, 'templates/talento_humano/director_th.html')

@login_required
@user_passes_test(lambda u: u.groups.filter(name= '').exists())
