...
class Usuario(models.Model):
    ...
    lugar_expedicion_documento = models.CharField(
    ...
    ultimo_nivel_estudio = models.CharField(
        max_length=255, null=True, blank=True)
    ...
    ultimo_grado_academico = models.CharField(
        max_length=255, null=True, blank=True)
    url_hoja_de_vida = models.URLField(
        blank=True, null=True)  # Enlace a Hoja de Vida
    # Pendiente - Rechazado - Aceptado
    estado_revision = models.CharField(max_length=50)
    # True = Activo - False = Inactivo
    activo = models.BooleanField(default=True)
    fk_creado_por = models.ForeignKey(
        settings.AUTH_USER_MODEL, related_name='usuarios_creados', null=True, blank=True, on_delete=models.SET_NULL)
    fecha_creacion = models.DateTimeField(auto_now_add=True)
    fk_modificado_por = models.ForeignKey(
        settings.AUTH_USER_MODEL, related_name='usuarios_modificados', null=True, blank=True, on_delete=models.SET_NULL)
    fecha_modificacion = models.DateTimeField(auto_now=True)
    # Relación con auth_user para definir si el usuario puede iniciar sesión en el aplicativo
    auth_user = models.OneToOneField(
        settings.AUTH_USER_MODEL, on_delete=models.SET_NULL, null=True, blank=True, related_name='usuario_app')
