...
class Estudiantes(models.Model):
    ...
    fk_tipo_documento = models.ForeignKey(
        TipoDocumento, on_delete=models.CASCADE, default=0)
...
class Matricula(models.Model):
    ...
    periodo = models.ForeignKey(
        Periodo, on_delete=models.CASCADE)  # Campo nuevo
...
class Prerrequisito(models.Model):
    materia = models.ForeignKey(
        Materia, on_delete=models.CASCADE, related_name='materia_principal')
    prerequisito = models.ForeignKey(
        Materia, on_delete=models.CASCADE, related_name='materia_requerida')

    def __str__(self):
        return f"{self.materia.codigo} requiere {self.prerequisito.codigo}"

    class Meta:
        db_table = 'prerrequisitos'
...
class MateriaAprobada(models.Model):
    ...
    estado_aprobacion = models.CharField(
        max_length=10, choices=ESTADO_OPCIONES)
